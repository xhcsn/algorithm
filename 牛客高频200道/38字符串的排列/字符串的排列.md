...
```java
//如果能够生成n-1个元素的全排列，就能生成n个元素的全排列。对于只有一个元素的集合，
//可以直接生成全排列，所以全排列的递归终止条件很明显，只有一个元素时。
//下面分析一下全排列的过程：
//首先，固定一个字符a，求后面两个字符的排列
//当两个字符bc排列求好之后，把第一个字符a和后面的b交换，得到bac，接着固定b求ac排列
//接下来把c放在第一个位置，但是此时需要先将a和b交换回来，再将a与c进行交换，求得排列
